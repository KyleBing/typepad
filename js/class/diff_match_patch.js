var diff_match_patch = function () {
  ...
  DIFF_INSERT = 1,
  DIFF_EQUAL = 0;
diff_match_patch.Diff = function (a, b, c, d) {
  this[0] = a;
  this[1] = b;
  this[2] = c;
  this[3] = d;
...
diff_match_patch.prototype.diff_main = function (a, b, c, d) {
  "undefined" == typeof d &&
    (d =
      0 >= this.Diff_Timeout
        ? Number.MAX_VALUE
        : new Date().getTime() + 1e3 * this.Diff_Timeout); //时间
  if (null == a || null == b) throw Error("Null input. (diff_main)");
  if (a == b) return a ? [new diff_match_patch.Diff(DIFF_EQUAL, a)] : [];
  "undefined" == typeof c && (c = !0);
  var e = c, //true
    l = this.diff_commonPrefix(a, b); //从前往后的索引！！！！
  c = a.substring(0, l); //change前的字
  a = a.substring(l);
  b = b.substring(l);
  f = this.diff_commonSuffix(a, b); //为0时====在末尾修改
  var g = a.substring(a.length - f); //change后的字
  a = a.substring(0, a.length - f); //删除的字
  b = b.substring(0, b.length - f); //新增的字
  a = this.diff_compute_(a, b, e, d, l, f); //生成现有字的数组
  c && a.unshift(new diff_match_patch.Diff(DIFF_EQUAL, c)); //新增后，从新增位置前，不变的部分拼接到前面
  g && a.push(new diff_match_patch.Diff(DIFF_EQUAL, g)); //新增后，从新增位置后，不变的部分拼接到后面
  this.diff_cleanupMerge(a);
  return a;
};
diff_match_patch.prototype.diff_compute_ = function (a, b, c, d, l, z) {
  if (!a) return [new diff_match_patch.Diff(DIFF_INSERT, b, l, z)];
  if (!b) return [new diff_match_patch.Diff(DIFF_DELETE, a, l, z)];
  ...
  return -1 != g
    ? ((c = [
        ...
        new diff_match_patch.Diff(DIFF_INSERT, e.substring(g + f.length), l, z),
      ]),
    ...
    ? [
        ...
        new diff_match_patch.Diff(DIFF_INSERT, b, l, z),
      ]
